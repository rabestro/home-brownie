"""Per-user conversation history for maintaining multi-turn context."""
...
class ConversationHistory:
    """Stores recent conversation turns for a single Telegram user.

    Each turn is a (user_message, bot_reply) pair. Older turns are dropped
    once MAX_TURNS is exceeded to keep token usage bounded.
    """
    ...
    def add(self, user_msg: str, bot_reply: str) -> None:
        """Appends a new turn and trims oldest if MAX_TURNS is exceeded."""
        self.turns.append((user_msg, bot_reply))
        if len(self.turns) > self.MAX_TURNS:
            self.turns.pop(0)

    def build_context(self, current_user_msg: str) -> str:
        """Builds a prompt string with prior conversation context prepended.

        Args:
            current_user_msg: The latest message from the user.

        Returns:
            Full prompt string including history.
        """
        if not self.turns:
